...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for roll in range(num_rolls):
        ...
        if(roll_num == 1):
            one_flag=True
        total_sum += roll_num
    return 1 if one_flag else total_sum
    # END PROBLEM 1
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    return roll_dice(num_rolls,dice) if num_rolls else picky_piggy(opponent_score)
    # END PROBLEM 3
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        score_change = cur_score - last_score
       
        if  score_change > new_high:
            new_high = score_change
            print(new_high, " point(s)! That's a record gain for Player ",who,'!', sep='')

        return announce_highest(who, cur_score, new_high)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    highest, high_roll = 0, 0
    for num_rolls in range(1,11):
        avg_rolls = make_averaged(roll_dice, trials_count)
        cur_avg = avg_rolls(num_rolls, dice)
        if cur_avg > highest:
            highest = cur_avg 
            high_roll = num_rolls
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    possible_points = picky_piggy(opponent_score)
    if possible_points + score == opponent_score:
        return 0 
